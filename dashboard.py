...
.pctleg{background:#f4f1e9;border:1px solid var(--line);border-radius:11px;padding:11px 13px;margin:0 0 6px}
.pctleg .lt{font-size:12px;color:var(--ink2);line-height:1.45}
.pctleg .lt b{font-weight:600}
.pctleg .scale{height:7px;border-radius:4px;margin:9px 0 4px;background:linear-gradient(90deg,#d3ccba,#8fb39b,#2f7d55)}
.pctleg .ticks{display:flex;justify-content:space-between;font-size:10.5px;color:var(--faint);font-weight:600}
.pctleg .ticks b{color:var(--ink2)}
...
var INV = {'Gols Sofridos':1,'xG Concedido':1,'Grandes Chances Concedidas':1,
  'Grandes Chances Perdidas':1,'Perdas de Bola':1,'Cartões Amarelos':1,
  'Cartões Vermelhos':1,'Erros que Levaram a Finalização':1,'Erros que Levaram a Gol':1};
...
/* Régua fixa por métrica: distribuição de TODOS os jogos-de-time da Copa
   (nunca filtrada — não muda com fase/exclusão). Usada para o percentil das
   barras do Perfil, que assim independem de quantos jogos foram selecionados. */
function buildDist(){
  DIST={}; var labels={};
  GROUPS.forEach(function(g){ g.stats.forEach(function(s){ labels[s]=1; }); });
  Object.keys(labels).forEach(function(lbl){
    DIST[lbl]=GAMES.map(function(g){return g.values[lbl];}).filter(function(v){return v!=null;});
  });
}
/* Percentil (0–1) da média-por-jogo do recorte na régua fixa. Para métricas
   "menor é melhor" (INV) inverte, então 1 = melhor sempre. NUNCA usa soma. */
function pctRank(t,label){
  var v=meanOf(teamGames(t),label);
  if(v==null) return {pct:0,val:null};
  var arr=DIST[label];
  if(!arr||arr.length<2) return {pct:.5,val:v};
  var below=0; for(var i=0;i<arr.length;i++){ if(arr[i]<v) below++; }
  var p=below/arr.length; if(INV[label]) p=1-p;
  return {pct:Math.max(0,Math.min(1,p)),val:v};
}
...
function renderTeam(){
  ...
  var groupsH=pg2.map(function(g){
    var rows=g.metrics.map(function(label){
      // Índices (IFE/IDO/P/xGD): ranking entre times (ratings, robustos a recorte).
      // Métricas de jogo: percentil na régua fixa da Copa (média-por-jogo do recorte).
      var isIdx=IDX.indexOf(label)>=0, v, pct, numTxt;
      if(isIdx){ v=valueFor(t,label); var rk=ordinal(t,label); pct=rk.pct; numTxt=rk.rank!=null?rk.rank+'º':'–'; }
      else { var pr=pctRank(t,label); v=pr.val==null?null:Math.round(pr.val*100)/100; pct=pr.pct; numTxt=pr.val==null?'–':Math.round(pct*100); }
      var barC=lerp('#d3ccba',ACC,pct), txtC=lerp('#9a9486',ACC,pct);
      var vStyle= isIdx||label==='Gols − xG'? 'color:'+signColor(v):'';
      return '<div class="statrow"><span class="sl">'+short(label)+qm(tip(label))+'</span>'+
        '<span class="track sm"><span class="fill" style="width:'+(pct*100)+'%;background:'+barC+'"></span></span>'+
        '<span class="sv" style="'+vStyle+'">'+disp(label,v)+'</span>'+
        '<span class="sr" style="color:'+txtC+'">'+numTxt+'</span></div>';
    }).join('');
  ...
  return '<section>'+
    ...
    '<div class="pbody"><div>'+
      ...
      '<div class="mgroups"><div class="overline" style="margin:0 2px 8px">Rendimento por métrica</div>'+
      '<div class="pctleg"><div class="lt">O número de cada <b>estatística</b> vai de <b>0 a 100</b>: é como o desempenho nos jogos selecionados se compara a um <b>jogo típico da Copa</b>. Não é posição no ranking — e <b>não muda</b> se você seleciona 1 ou vários jogos. Nas métricas em que menos é melhor (gols sofridos, cartões…), 100 continua sendo o melhor.'+
        '<div class="scale"></div><div class="ticks"><span>0 · <b>entre os piores</b></span><span>50 · típico</span><span><b>100 · entre os melhores</b></span></div>'+
        '<div style="font-size:11px;color:var(--faint);margin-top:7px">Já os <b>índices</b> (IFE, IDO, P de vitória, xGD) mostram a posição da seleção entre as '+ifeR.n+' do torneio — por isso vêm com <b>º</b>.</div></div>'+
      groupsH+
