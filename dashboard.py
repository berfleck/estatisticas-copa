...
var DIMS = [
  ...
  {name:'Finalização', comps:[['@xg_por_fin',.50],['@sot_pct',.25],['Grandes Chances Convertidas',.25]]},
  {name:'Construção',  comps:[['Passes Certos',.45],['Posse de Bola (%)',.35],['Passes',.20]]},
  {name:'Pressão',     comps:[['Toques na Área',.30],['Chutes Dentro da Área',.20],['Entradas no Último Terço',.20],['Ações no Último Terço',.15],['Faltas Sofridas no Último Terço',.15]]},
  {name:'Defesa',      comps:[['xG Concedido',.30,true],['Grandes Chances Concedidas',.20,true],['Gols Evitados',.12],['Defesas Difíceis',.08],['Desarmes',.10],['Interceptações',.10],['Recuperações de Bola',.06],['Duelos Ganhos (%)',.04]]}
];
var CB = [
  {name:'Ataque',     metrics:['xG (Gols Esperados)','Finalizações','Chutes no Alvo','Grandes Chances','Toques na Área']},
  {name:'Construção', metrics:['Posse de Bola (%)','Passes Certos','Duelos Ganhos (%)','Dribles (certos)']},
  {name:'Defesa',     metrics:['Desarmes','Interceptações','Recuperações de Bola','Gols Evitados']}
];

var state = {
  mode:'painel', phase:'all', sortKey:'IFE', customMetric:null,
  metricMenuOpen:false, metricQuery:'', agg:'media',
...
function disp(label,v){ if(v==null) return '–'; if(label==='IFE'||label==='xGD') return signed(v,2); if(label==='IDO') return signed(v,0); return fmt(v)+(label.indexOf('(%)')>=0?'%':''); }
...
function ordinal(t,label){ var arr=teamsInPhase().map(function(x){return {x:x,v:valueFor(x,label)};}).filter(function(o){return o.v!=null;}).sort(function(a,b){return b.v-a.v;}); var i=-1; for(var k=0;k<arr.length;k++) if(arr[k].x===t){i=k;break;} return {rank:i<0?null:i+1,n:arr.length,pct:i<0?0:(arr.length>1?(arr.length-1-i)/(arr.length-1):1)}; }
...
function compVal(row,key){
  ...
  if(key==='@sot_pct'){ var s=V['Chutes no Alvo']; return (f&&s!=null)? s/f : null; }
  if(V[key]!=null) return V[key];
  /* concedidos podem não vir no payload: cai para a linha do adversário */
...
function radarSVG(sel){
  ...
  DIMS.forEach(function(dim,i){ var c=Math.cos(ang(i)),s=Math.sin(ang(i)); var lx=cx+(R+17)*c,ly=cy+(R+17)*s;
    ...
    out+='<text x="'+lx.toFixed(1)+'" y="'+ly.toFixed(1)+'" text-anchor="middle" dominant-baseline="middle" font-size="10" font-family="Archivo" font-weight="600" fill="#6b665b">'+dim.name+'</text>';
  });
...
function renderPainel(){
  ...
  rows.sort(function(a,b){ var va=sk==='IFE'?a.ife:sk==='IDO'?a.ido:a.cust, vb=sk==='IFE'?b.ife:sk==='IDO'?b.ido:b.cust; return (vb==null?-1e9:vb)-(va==null?-1e9:va); });
...
function renderMetricMenu(){
  var q=state.metricQuery.trim().toLowerCase();
  ...
  GROUPS.forEach(function(g){
    var sts=g.stats.filter(function(l){ return IDX.indexOf(l)<0 && (!q||l.toLowerCase().indexOf(q)>=0); });
...
function renderCompare(){
  ...
  if(sel.length<2){
    ...
    return '<section>'+titleRow+'<div class="empty"><div class="et">Escolha ao menos duas seleções para comparar</div><div>Sugestões: '+sugH+'</div></div></section>';
  }
  ...
  var bars=CB.map(function(g){
    var rows=g.metrics.map(function(label){
      ...
      var lead=Math.max.apply(null,cells.map(function(c){return c.v==null?-1e9:c.v;}));
      ...
      return '<div class="mrow"><div class="mlbl" title="'+esc(tip(label))+'">'+label+'</div>'+cellsH+'</div>';
    }).join('');
  ...
  return '<section>'+titleRow+
    ...
    '</div><div>'+bars+'</div></div></section>';
}
function renderAddMenu(teams,sel){
  var q=state.compareQuery.trim().toLowerCase(); if(!q) return '';
  var res=teams.filter(function(t){return sel.indexOf(t)<0&&t.toLowerCase().indexOf(q)>=0;}).slice(0,8);
...
function renderTeam(){
  ...
  if(!state.team){
    ...
    var list=teams.filter(function(t){return !q||t.toLowerCase().indexOf(q)>=0;}).map(function(t){ var r=record(t);
  ...
  var groupsH=pg2.map(function(g){
    var rows=g.metrics.map(function(label){ var v=valueFor(t,label); var rk=ordinal(t,label);
      ...
      var vStyle= IDX.indexOf(label)>=0? 'color:'+signColor(v):'';
      return '<div class="statrow"><span class="sl" title="'+esc(tip(label))+'">'+short(label)+'</span>'+
  ...
  return '<section>'+
    ...
    '<div class="pbody"><div>'+
      ...
      '<div class="mgroups"><div class="overline" style="margin:0 2px 4px">Rendimento por métrica <span style="text-transform:none;letter-spacing:0;color:var(--muted);font-weight:500">· posição entre '+ifeR.n+' seleções</span></div>'+groupsH+'</div>'+
    '</div></section>';
}
